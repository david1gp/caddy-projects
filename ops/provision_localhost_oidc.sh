#!/usr/bin/env bash
# Provision Zitadel confidential OIDC client for local-dev caddy-projects.
# Safe to re-run; credentials preserved; redirect URIs updated in place.
#
# Zitadel: non-HTTPS localhost redirects require devMode=true on the app
# (set in the create/update payload below). Without it, Zitadel rejects
...
ENV_FILE="${LOCALHOST_OIDC_ENV_FILE:-$HOME/.config/caddy-projects/localhost.oidc.env}"
API="https://auth.contentoren.de"
ORG_NAME="Contentoren"
PROJECT_NAME="caddy-projects"
APP_NAME="caddy-projects localhost"
DRY_RUN=0
PORT=2015

usage() {
	cat <<'EOF'
Usage: provision_localhost_oidc.sh [options]

Provision a Zitadel OIDC client for local-dev caddy (localhost redirects).

Options:
  --port <n>  Local HTTP port for callback (default: 2015)
  --dry-run   Print redirect URIs and planned API calls; touch nothing
  -h, --help  Show this help

Env overrides:
  ZITADEL_ENV                 path to zitadel.env (IDP_ADMIN_PAT)
  LOCALHOST_OIDC_ENV_FILE     output env file path
EOF
}
...
if [[ "$DRY_RUN" -eq 1 ]]; then
	...
	echo "[dry-run] port=$PORT devMode=true (localhost http redirects)"
...
oidc_body() {
	# devMode=true: allow non-HTTPS redirect URIs for localhost
...
echo "---"
echo "Provisioned Zitadel OIDC client for caddy-projects localhost"
