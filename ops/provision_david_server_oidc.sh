#!/usr/bin/env bash
# Provision Zitadel confidential OIDC client for caddy-projects on david-server.
# Safe to re-run; credentials preserved; redirect URIs updated in place.
#
# Redirect URIs sourced from david-server/caddy_snippet/david.Caddyfile
# (and the matching tail of leo-server Caddyfile):
#   opencode.david-siewert.com, oc.david-siewert.com, o.david-siewert.com
#   preview.rift-command.com
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DAVID_SERVER_DIR="${DAVID_SERVER_DIR:-$HOME/Coding/leo_internal/david-server}"
...
ENV_FILE="${DAVID_SERVER_OIDC_ENV_FILE:-$DAVID_SERVER_DIR/caddy_config/caddy-projects.oidc.env}"
API="https://auth.contentoren.de"
ORG_NAME="Contentoren"
PROJECT_NAME="caddy-projects"
APP_NAME="caddy-projects david-server"
DRY_RUN=0

REDIRECT_URIS=(
	"https://opencode.david-siewert.com/oauth2/callback"
	"https://oc.david-siewert.com/oauth2/callback"
	"https://o.david-siewert.com/oauth2/callback"
	"https://preview.rift-command.com/oauth2/callback"
)
POST_LOGOUT_URIS=(
	"https://opencode.david-siewert.com/"
	"https://oc.david-siewert.com/"
	"https://o.david-siewert.com/"
	"https://preview.rift-command.com/"
)

usage() {
	cat <<'EOF'
Usage: provision_david_server_oidc.sh [options]

Provision a Zitadel OIDC client for caddy-projects on david-server.
...
Env overrides:
  ...
  DAVID_SERVER_OIDC_ENV_FILE    output env file path
  DAVID_SERVER_DIR              david-server repo root
EOF
}
...
echo "---"
echo "Provisioned Zitadel OIDC client for caddy-projects david-server"
